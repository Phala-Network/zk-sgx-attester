...
fn decode_auth_data(ver: u16, input: &mut &[u8]) -> Result<AuthData, scale_codec::Error> {
    match ver {
        3 => {
        ...
        _ => Err(scale_codec::Error::from("unsupported quote version")),
    }
}
    ...
    fn decode<I: Input>(input: &mut I) -> Result<Self, scale_codec::Error> {
        ...
        if header.version > 4 {
            ...
            if body.body_type != BODY_SGX_ENCLAVE_REPORT_TYPE {
                return Err(scale_codec::Error::from("unsupported body type"));
            }
